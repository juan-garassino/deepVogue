...
def get_z(shape, seed=None, uniform=False):
    ...
    if uniform:
        return rnd.uniform(0.0, 1.0, shape)
    else:
        return rnd.randn(*shape)  # *x unpacks tuple/list to sequence


def smoothstep(x, NN=1.0, xmin=0.0, xmax=1.0):
    N = math.ceil(NN)
    x = np.clip((x - xmin) / (xmax - xmin), 0, 1)
    result = 0
    for n in range(0, N + 1):
        result += (
            scipy.special.comb(N + n, n)
            * scipy.special.comb(2 * N + 1, N - n)
            * (-x) ** n
        )
    result *= x ** (N + 1)
    if NN != N:
...
def lerp(z1, z2, num_steps, smooth=0.0):
    vectors = []
...
def slerp(z1, z2, num_steps, smooth=0.0):
...
def latent_anima(
    ...
    if gauss:
        ...
        lats_post = (
            lats_post / np.linalg.norm(lats_post, axis=-1, keepdims=True)
        ) * math.sqrt(np.prod(shape))
        log += ", gauss"
        latents = lats_post
...
def multimask(x, size, latmask=None, countHW=[1, 1], delta=0.0):
    Hx, Wx = countHW
    bcount = x.shape[0]
    ...
    if max(countHW) > 1:
        ...
        if Wx > 1:
            ...
            for i in range(Wx):
                ch_mask = (
                    peak_roll(W, Wx, i, delta).unsqueeze(0).unsqueeze(0)
                )  # [1,1,w] th
        ...
        if Hx > 1:
            ...
            for i in range(Hx):
                ch_mask = (
                    peak_roll(H, Hx, i, delta).unsqueeze(1).unsqueeze(0)
                )  # [1,h,1] th
...
def peak_roll(width, count, num, delta):
    ...
    if num == 0:
        shift = max(width - (step // 2), 0.0)  # must be positive!
    else:
...
def peak(steps, delta):
    x = torch.linspace(0.0 - delta, 1.0 + delta, steps)
...
def calc_init_res(shape, resolution=None):
    if len(shape) == 1:
        ...
        shape = [*shape, 1]
    size = (
        shape[:2] if shape[2] < min(*shape[:2]) else shape[1:]
    )  # fewer colors than pixels
...
def img_list(path, subdir=None):
    ...
    files = [
        f
        for f in files
        if os.path.splitext(f.lower())[1][1:] in ["jpg", "jpeg", "png", "ppm", "tif"]
    ]
    return sorted([f for f in files if os.path.isfile(f)])

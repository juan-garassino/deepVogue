...
- **First model:** `interstella256` — Interstella 5555 frames at 256px/2fps, transfer from `stylegan3-t-ffhqu-256x256.pkl`, batch 16, gamma 1.0, **cbase 16384** (NVIDIA's 256px pretrained pkls are half-capacity; `DV_CBASE` must match the resume pkl or net construction fails on shape mismatch).
- **Unattended chaining:** Cloud Scheduler `deepvogue-train-chain` (`0 * * * *`, europe-west1) fires one slice hourly, triggering the job's `:run` via the compute default SA (which already has `roles/editor` → `run.jobs.run`; no new IAM). Two guards make it safe to leave running: `DV_TARGET_KIMG` no-ops slices once cumulative kimg (completed-slice count × `DV_KIMG`) reaches the cap, and on reaching it the slice **pauses its own scheduler** (`DV_SCHEDULER` + `GCP_PROJECT`/`GCP_REGION`, via the SA's editor role) so no further no-op fires bill. Manual controls: `gcloud scheduler jobs pause|resume deepvogue-train-chain --location=europe-west1`. Note: deepVogue and deep-sculpt share the project's L4 quota; hourly cadence vs ~52-min slices leaves an ~8-min buffer. **Gotcha:** don't manually `train-slice` *and* have the scheduler enabled — an off-cadence manual fire overlaps the on-the-hour fire (cancel the redundant one with `gcloud run jobs executions cancel`).
- **Fine-tune:** re-point the same job to a new `DV_RUN_URI` + `DV_RESUME_FROM=<final checkpoint>` + `DV_MIRROR=true` (x-flip aug, 2× data — the one projection-safe per-frame augmentation) + a smaller `DV_TARGET_KIMG`, then resume the scheduler. `interstella256_ft` fine-tunes `interstella256`'s 512-kimg model this way.

### Latent-cinema jobs (projection + walk, L4)

Two more Cloud Run L4 jobs render the film from a trained model (same image, command overridden):
- **`deepvogue-project`** (`infra/cloudrun/project.job.yaml`, `make deploy-project-job`/`project-run`) inverts every `DV_PROJ_STRIDE`-th film frame into the generator's W+ space via `deepVogue.cinema project-frames`. Resumable (skips existing `projected_w.npz`), anchors mirror to `DV_ANCHORS_URI`. **Anchors are generator-specific** — re-project after any retrain. `DV_NETWORK_URI` picks the model. Stride 4 → ~1,349 anchors (~7 slices); seed a finer run from a coarser one's dir since coarse strides are subsets.
- **`deepvogue-walk`** (`infra/cloudrun/walk.job.yaml`) pulls model + anchors + `frames_index.json` and runs `deepVogue.walk` in temporal order → mp4 to `DV_WALK_URI`. Rendering ~10k frames is minutes on L4 vs hours on CPU. `DV_FPS_PER_SEG` controls pacing (8 = readable dwell on each anchor).
- **Finding:** dense anchors fix *narrative legibility*; per-frame *sharpness* is capped by 256px + half-capacity (mirror/crops can't add detail; crops break full-frame projection). 512px retrain is the sharpness lever.

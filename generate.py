...
def interpolate(
    ...
    # generate frames
    images(
        G, device, points, space, truncation_psi, label, noise_mode, outdir, start, stop
    )
...
def truncation_traversal(
    G, device, z, label, start, stop, increment, noise_mode, outdir
):
    count = 1
    trunc = start
...
@click.option(
    "--diameter", type=float, help="diameter of loops", default=100.0, show_default=True
)
@click.option(
    "--frames",
    type=int,
...
@click.option(
    "--fps", type=int, help="framerate for video", default=24, show_default=True
)
@click.option(
    "--increment",
    type=float,
    help="truncation increment value",
    default=0.01,
    show_default=True,
)
@click.option(
    "--interpolation",
    type=click.Choice(["linear", "slerp", "noiseloop", "circularloop"]),
    default="linear",
...
@click.option(
    "--process",
    type=click.Choice(
        ["image", "interpolation", "truncation", "interpolation-truncation"]
    ),
    default="image",
...
@click.option(
    "--random_seed",
    type=int,
    help="random seed value (used in noise and circular loop)",
    default=0,
    show_default=True,
)
@click.option(
...
@click.option(
    "--start",
    type=float,
    help="starting truncation value",
    default=0.0,
    show_default=True,
)
@click.option(
    "--stop",
    type=float,
    help="stopping truncation value",
    default=1.0,
    show_default=True,
)
@click.option(
...
def generate_images(
    ...
    if process == "image":
        ...
        if seeds is not None:
            ...
            vidname = (
                f"{process}-{interpolation}-{diameter}dia-seed_{random_seed}-{fps}fps"
            )
        ...
        seed = seeds[0]
        vidname = (
            f"{process}-seed_{seed}-start_{start}-stop_{stop}-inc_{increment}-{fps}fps"
        )

        # generate frames
        truncation_traversal(
            G, device, seeds, label, start, stop, increment, noise_mode, dirpath
        )

    ...
    *,
    ...
    if G.mapping.c_dim:
        labels = torch.from_numpy(
            0.5 * np.random.RandomState(123).randn(w_avg_samples, G.mapping.c_dim)
        ).to(device)
    ...
    if initial_latent is not None:
        ...
        if kmeans and use_clip and target_text is not None:
            ...
            kmeans_latents = torch.tensor(
                cluster_centers, dtype=torch.float32, device=device, requires_grad=True
            )
    ...
    if target_image is not None:
        ...
        if use_center:
            center_target = F.interpolate(target_images, size=(448, 448), mode="area")[
                :, :, 112:336, 112:336
            ]
    ...
    if use_vgg:
        vgg_target_features = vgg16(
            target_images, resize_images=False, return_lpips=True
        )
        if use_center:
            vgg_target_center = vgg16(
                center_target, resize_images=False, return_lpips=True
            )
            ...
            with torch.no_grad():
                ...
                if use_center:
                    clip_target_center = model.encode_image(
                        ((center_target / 255.0) - image_mean[None, :, None, None])
                        / image_std[None, :, None, None]
                    ).float()

    if kmeans_latents is not None and use_clip and target_text is not None:
        scores, kmeans_images = score_images(
            G,
            model,
            target_text,
            kmeans_latents.repeat([1, G.mapping.num_ws, 1]),
            device=device,
        )
        ind = np.argpartition(scores, 4)[:4]
        w_avg = torch.median(kmeans_latents[ind], dim=0, keepdim=True)[0].repeat(
    ...
    for step in range(num_steps):
        ...
        synth_images = G.synthesis(
            torch.clamp(ws, -latent_range, latent_range), noise_mode="const"
        )
        ...
        if use_center:
            center_synth = F.interpolate(synth_images, size=(448, 448), mode="area")[
                :, :, 112:336, 112:336
            ]
        ...
        if use_clip:
            ...
            if target_text is not None:
                ...
                if use_center:
                    clip_text += (
                        1 - model(clip_cynth_center_image, target_text)[0].sum() / 100
                    )
                dist += 2 * F.relu(
                    adj_center * clip_text * clip_text - min_threshold / adj_center
                )
...
@click.option(
    "--target-text", help="Target text to project to", required=False, default=None
)
...
@click.option(
    "--use-pixel",
    help="Use L1/L2 distance on pixels in the loss",
    type=bool,
    default=True,
    show_default=True,
)
@click.option(
    "--use-penalty",
    help="Use a penalty on latent values distance from the mean in the loss",
    type=bool,
    default=True,
    show_default=True,
)
@click.option(
    "--use-center",
    help="Optimize against an additional center image crop",
    type=bool,
    default=True,
    show_default=True,
)
@click.option(
    "--use-kmeans",
    help="Perform kmeans clustering for selecting initial latents",
    type=bool,
    default=True,
    show_default=True,
)
def run_projection(
    ...
    if target_fname:
        target_pil = (
            PIL.Image.open(target_fname).convert("RGB").filter(ImageFilter.SHARPEN)
        )

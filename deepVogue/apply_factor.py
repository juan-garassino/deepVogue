...
def generate_images(z, label, truncation_psi, noise_mode, direction, file_name):
    if args.space == "w":
        ...
        img2 = G(
            z + direction, label, truncation_psi=truncation_psi, noise_mode=noise_mode
        )
        img3 = G(
            z - direction, label, truncation_psi=truncation_psi, noise_mode=noise_mode
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--index", type=num_range, default="-1", help="index of eigenvector"
    )
    parser.add_argument(
        "--seeds",
        type=num_range,
        default="r1",
        help="list of random seeds or 'r10' for 10 random samples",
    )
    parser.add_argument(
        "-d",
        "--degree",
    ...
    parser.add_argument(
        ...
        type=str,
        default="/cff_output/",
        help="directory for result samples",
    )
    parser.add_argument(
        "--ckpt", type=str, required=True, help="stylegan2-ada-pytorch checkpoints"
    )
    parser.add_argument(
        "--space",
        type=str,
        default="w",
        help="generate images in the w space or z space",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="increment degree for interpolation video",
    )
    ...
    device = torch.device("cuda")
    with neuronal_network_utils.util.open_url(args.ckpt) as f:
    ...
    if isinstance(seeds, int):
        for i in range(seeds):
            ...
            )  # 2**32-1 is the highest seed value
        mode = "random"
    ...
    for l in latents:
        ...
        for j in index_list_of_eigenvalues:
            ...
            image_group = generate_images(
                z, label, truncation_psi, noise_mode, direction, file_name
            )
    ...
    if args.vid:
        ...
        folder_name = (
            f"seed-{str_seed_list}_index-{str_index_list}_degree-{args.degree}"
        )
        ...
        for l in latents:
            ...
            for j in index_list_of_eigenvalues:
                ...
                if args.space == "w":
                    ...
                    pts = line_interpolate(
                        ws, int((args.degree * 2) / args.vid_increment)
                    )
                else:
                    pts = line_interpolate(
                        [z - direction, z + direction],
                        int((args.degree * 2) / args.vid_increment),
                    )

                fcount = 0
                for pt in pts:
                    img = generate_image(
                        pt, label, truncation_psi, noise_mode, args.space
                    )
                    grid = utils.save_image(
                        img,
                        ...
                        value_range=(
                            -1,
                            1,
                        ),  # change range to value_range for latest torchvision
                        nrow=1,
                    )
                    fcount += 1

...
from deepVogue.pytorch_utils.ops import conv2d_gradfix
from deepVogue.pytorch_utils.ops import grid_sample_gradfix
...
def training_loop(
    ...
    if rank == 0:
        print("\nℹ️ " + f"Distributing across {num_gpus} GPUs...")
    ddp_modules = dict()
    for name, module in [
        ("G", G),
        ("G_mapping", G.mapping),
    ...
    # Train.
    if rank == 0:
        print("\nℹ️ " + f"Training for {total_kimg} kimg...")
        print()
    cur_nimg = nimg
    cur_tick = 0
    ...
    while True:
        ...
        if (
            ...
            save_image_grid(
                images,
                os.path.join(run_dir, f"fakes{cur_nimg//1000:06d}.jpg"),
                drange=[-1, 1],
                grid_size=grid_size,
            )

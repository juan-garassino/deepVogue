...
class MetricOptions:
    def __init__(
        ...
        self.G = G
        self.G_kwargs = neuronal_network_utils.EasyDict(G_kwargs)
        self.dataset_kwargs = neuronal_network_utils.EasyDict(dataset_kwargs)
...
def get_feature_detector(
    ...
    if key not in _feature_detector_cache:
        ...
        with neuronal_network_utils.util.open_url(url, verbose=(verbose and is_leader)) as f:
...
class FeatureStats:
    ...
    def load(pkl_file):
        with open(pkl_file, "rb") as f:
            s = neuronal_network_utils.EasyDict(pickle.load(f))
...
class ProgressMonitor:
    ...
    def update(self, cur_items):
        ...
        if (self.verbose) and (self.tag is not None):
            print(
                f"{self.tag:<19s} items {cur_items:<7d} time {neuronal_network_utils.util.format_time(total_time):<12s} ms/item {time_per_item*1e3:.2f}"
            )
...
def compute_feature_stats_for_dataset(
    ...
    **stats_kwargs,
    ...
    dataset = neuronal_network_utils.util.construct_class_by_name(**opts.dataset_kwargs)
    ...
    if opts.cache:
        ...
        cache_file = neuronal_network_utils.make_cache_dir_path("gan-metrics", cache_tag + ".pkl")

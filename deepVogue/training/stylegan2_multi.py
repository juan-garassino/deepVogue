...
def modulated_conv2d(
    ...
    splitfine=0.0,  # frame split edge fineness (float from 0+)
...
class SynthesisLayer(torch.nn.Module):
    def __init__(
        ...
        if use_noise:
            ...
            self.register_buffer(
                "noise_const",
                torch.randn(
                    [resolution * init_res[0] // 4, resolution * init_res[1] // 4]
                ),
            )
    ...
    def forward(self, x, w, noise_mode="random", fused_modconv=True, gain=1):
        ...
        if self.use_noise and noise_mode == "random":
            ...
            noise = (
                torch.randn([x.shape[0], 1, *sz], device=x.device) * self.noise_strength
            )
        ...
        if self.use_noise and noise_mode == "const":
            ...
            noise_size = (
                self.size
                if self.up == 2 and self.size is not None and self.resolution > 4
                else x.shape[2:]
            )
            noise = fix_size(
                noise.unsqueeze(0).unsqueeze(0), noise_size, scale_type=self.scale_type
            )[0][0]
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(
        ...
        **block_kwargs,  # Arguments for SynthesisBlock.
        ...
        if verbose:
            print(hws, "..", custom_res, self.res_log2 - 1)

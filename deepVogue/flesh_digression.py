...
def circular_interpolation(
    radius: float,
    latents_persistent: Tuple[np.ndarray, np.ndarray, np.ndarray],
    circle_pos: float,
) -> np.ndarray:

    latents_a, latents_b, latents_c = latents_persistent

    latents_axis_x = (latents_a - latents_b).flatten() / linalg.norm(
        latents_a - latents_b
    )
    latents_axis_y = (latents_a - latents_c).flatten() / linalg.norm(
        latents_a - latents_c
    )
...
def image_from_latent(
    G: torch.nn.Module, psi: float, z: np.ndarray, device: torch.device
) -> np.ndarray:
...
def generate_from_generator_adaptive(
    psi: float,
    radius_large: float,
    radius_small: float,
    step1: float,
    ...
    current_latent = circular_interpolation(
        radius_small, latents_persistent_small, current_position
    )
    ...
    while current_position_style < video_length:
        ...
        w = next(
            layer
            for name, layer in G.named_parameters()
        ...
        with torch.no_grad():
            v2 += torch.from_numpy(
                circular_interpolation(
                    radius_large, latents_persistent_large, current_position + np.pi
                )
            ).to(device)
        ...
        current_latent = circular_interpolation(
            radius_small, latents_persistent_small, current_position_style
        )
        current_image = image_from_latent(G, psi, current_latent, device)
...
def main(
    ...
    with neuronal_network_utils.util.open_url(pkl) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pkl", help="A .pkl of a StyleGAN network model", required=True
    )
    parser.add_argument(
        "--psi",
        help="The truncation psi used in the generator",
        default=0.7,
        type=float,
    )
    parser.add_argument(
        "--radius_large",
        help="The radius for the constant layer interpolation",
        default=300.0,
        type=float,
    )
    parser.add_argument(
        "--radius_small",
        help="The radius for the latent space interpolation",
        default=40.0,
        type=float,
    )
    parser.add_argument(
        "--step1",
        help="The value of the step/increment for the constant layer interpolation",
        default=0.005,
        type=float,
    )
    parser.add_argument(
        "--step2",
        help="The value of the step/increment for the latent space interpolation",
        default=0.0025,
        type=float,
    )
    parser.add_argument(
        "--seed", help="Seed value for random state", default=None, type=int
    )
    parser.add_argument(
        "--seeds",
        help="Three comma separated seed values for circluar interpolation",
        default=None,
        type=seed_values,
    )
    parser.add_argument(
        "--size", help="Size of output (in format x-y)", default=None, type=size_range
    )
    parser.add_argument(
        "--scale_type",
        help="Options: pad, padside, symm, symmside",
        default="pad",
        type=str,
    )
    parser.add_argument(
